...
- 保留已确认的正文字号，将研究、论文、软件和职业经历重构为大幅标题与主题插画相结合的章节。
- 用原创 SVG 卡通研究者替换个人照片：随着滚动切换 DNA、读书、电脑和问候姿态；宽屏停留在侧边留白，小屏收进导航。支持减少动态效果及无 JavaScript 阅读。
- 放大研究路径节点与箭头，重新调整桌面和手机的流程布局。
- 按要求删除 An open notebook 区块及其三个链接，移除已停用的 Cloud disk 入口。已有 AI 指南文件继续保留。
- 保留论文信息、项目贡献、协作者署名和 HKU 任职时间；示意单倍型矩阵明确标为说明用图。

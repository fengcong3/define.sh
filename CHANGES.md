...
- 将角色发型更新为微分碎盖，采用蓬松轮廓、轻微分缝与细碎刘海；所有场景共用新的 SVG。
- 将 One tiny experiment? 改为从头顶弹出的对话气泡，首屏伸出圆形背景；支持键盘焦点、减少动画及缩小角色后的边界适配。

- 参考提供的发型图，细化角色的层次侧分、面部、衣物与各场景道具；增加悬停目光跟随、表情与招手反馈。
- 点击角色可打开“口袋温室”，选择亲本基因型并抽样生成豌豆后代，展示预期概率和本次收获；支持键盘、触摸、减少动画和关闭时取消未完成操作。
